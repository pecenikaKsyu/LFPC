...
class CFG:
    ...
    def removeDeletedSymbols(self):
        ...
        # i.e. productions that appear in non-terminals that aren't in self.nonterminals
        good = False
        while not good:
            good = True
        ...
        while not ok:
            ...
            for k, v in self.prods.items():
                ...
                for string in v:
                    ...
                    for nonterminal in newNonterminals:  # check not to duplicate non-terminals
                    ...
                    if newNonterminal is None:  # if no simple non-terminals, create
                        newNonterminal = self.__findFreeNonTerms__(nonterminals[1][0])  # alike A_33
    ...
    def removeEmptyProds(self):
        ...
        # productions form: A -> a($), A -> B or A -> BC
    ...
    def __convertToCNF__(self):
        # 1. remove the unreachable and unproductive elements
        self.removeRedundantSymbols()
        # 2. substitute the terminals in all productions, unless A -> a
        # for each terminal create an accessible non-terminal symbol
        self.correctTheTerms()
        # 3. correct productions with more than 2 non-terminals by 2 non-terminals
        self.renameLongerThan2()
        # 4. delete lambda-productions
